...
[Sophie Nabitz](mailto:sophie.nabitz@umontpellier.fr),
[Cyrille Nadal](mailto:cyrille.nadal@umontpellier.fr),
[Victor Poupet](mailto:victor.poupet@umontpellier.fr),
[Petru Valicov](mailto:petru.valicov@umontpellier.fr)
* Le [forum Piazza](https://piazza.com/umontpellier.fr/spring2026/devobjets) de ce cours pour poser vos questions
...
Les instructions de ce TP et de ceux qui vont suivre sont destinées aux utilisateurs Linux (Ubuntu ou autre). Nous vous encourageons à utiliser Linux, car durant toute votre formation vous en aurez besoin et plus généralement en tant qu'informaticien, il est important d'être familiarisé avec.
...
Voici un petit récapitulatif pour voir si vous avez bien suivi :

* Qu'est-ce qu'un _dépôt Git_ ?
...
* À quoi correspond le fichier `.gitignore` ?
* À quoi correspond le raccourci `git push` ?
* À quoi correspond le raccourci `git pull` ?
* Qu'est-ce qu'un _conflit_ Git ?

Pensez à écrire vos réponses sur une feuille de papier et à en discuter avec votre enseignant. Vous en aurez besoin pour l'interro !
...
**Remarque** : généralement, selon la configuration initiale, la branche principale d'un dépôt est nommée soit `main`, soit (selon une pratique plus ancienne) `master`, soyez attentif au nom que vous utilisez par la suite dans vos commandes.
...
1. **Dans un terminal**, compilez, exécutez et vérifiez votre programme :
    ```sh
    ~/Dev-Objets/tp1/Exercice1$ javac HelloWorld.java
    ~/Dev-Objets/tp1/Exercice1$ java HelloWorld
    ```
2. Un fichier `HelloWorld.class` contenant le bytecode a été créé.
3. Versionnez l'intégralité de votre travail avec `git add .` + `git commit`.
4. Faites un `git push` sur le dépôt distant GitLab et comparez avec votre répertoire local. Quelle différence constatez-vous entre votre répertoire local et le dépôt distant ? Pourquoi ?

En règle générale, une séparation entre le code source et le bytecode est nécessaire pour une meilleure lisibilité de votre programme. Ce qui est généralement conseillé, c'est d'avoir au moins deux répertoires à la racine de votre projet. Par exemple `src` + `bin` ou encore `src` + `target`. Pour les différentes options de compilation : https://docs.oracle.com/en/java/javase/21/docs/specs/man/javac.html
...
4. Pour simuler le travail d'un collègue sur une autre machine, clonez une nouvelle copie du fork GitLab '*dev-objets/etu/votreUsername/tp1*' dans un répertoire différent :
    ```sh
    ~/Dev-Objets/$ git clone git@gitlabinfo.iutmontp.univ-montp2.fr:dev-objets/etu/votreUsername/TP1.git copie_TP1
    ```
    Un nouveau répertoire `copie_TP1` a été créé. Il contient la version du code de l'exercice 1. Modifiez son fichier  `HelloWorld.java` afin d'afficher le message _"Je suis une autre version modifiée du code de l'exo 1"_. Compilez et exécutez pour vérifier.
...
8. Bien entendu, il s'agit d'un conflit et Git ne vous laissera pas faire des bêtises gratuitement :smirk:
  Le message d'erreur devrait ressembler à quelque chose comme ceci :
    ...
    Il y a plusieurs façons de résoudre le conflit en fonction du contexte et du besoin de l'utilisateur. Vous serez amené à les découvrir par vous-même tout au long du semestre. Dans notre cas, nous avons un **"vrai"** conflit : la même ligne de code est modifiée différement dans les dépôts locaux `copie_tp1` et dans le dépôt distant `gitlabinfo.iutmontp.univ-montp2.fr:dev-objets/etu/votreUsername/TP1`. En effet, dans cette situation Git ne peut pas résoudre le conflit sans consigne précise : puisque les deux versions se contredisent, laquelle est la bonne ?
    
    Indiquons à Git d'aller la chercher les informations du dépôt distant **sans modifier votre code local** (`git fetch`) :

    ```sh
    ~/Dev-Objets/tp1$ git fetch origin
    ```
    **Remarque** : cette commande télécharge les changements distants (sur `origin`) mais ne touche pas à vos fichiers locaux. C'est donc une opération **sûre**.
9. Comparons l'état des commits de votre version locale avec la version distante

    ```sh
    ~/Dev-Objets/tp1$ git log --oneline --graph --all
    ```
    
    Vous verrez apparaître dans le terminal l'historique des commits avec le conflit représenté sous forme de 2 branches qui divergent :

    ```sh
    * b11e5f6 (origin/main) Message du commit de copie_TP1
    | * 7423229 (HEAD -> main) Message du commit sur le dépôt distant
    |/  
    * 51a095c Commit précédent commun
    ```
10. Regardons les différences entre les deux versions

    ```sh
    ~/Dev-Objets/tp1$ git diff main origin/main
    ```
11. Supposons que la bonne version soit celle du **dépôt distant**. Essayons de fusionner :

    ```sh
    ~/Dev-Objets/tp1$ git merge origin/main
    ```

    Git detecte le conflit (impossible à résoudre automatiquement sans votre aide) :

    ```sh
    Auto-merging Exercice1/HelloWorld.java
    CONFLICT (content): Merge conflict in Exercice1/HelloWorld.java
    Automatic merge failed; fix conflicts and then commit the result.
    ```
12. Vérifions l'état

    ```sh
    ~/Dev-Objets/tp1$ git status
    ```
    ...
    both modified:   Exercice1/HelloWorld.java
    
    no changes added to commit (use "git add" and/or "git commit -a")
    ```

13. Résolvons le conflit. Ouvrez le fichier `HelloWorld.java` votre éditeur de texte favori. Vous verrez ce genre de chose :
    ...
    =======
            System.out.println("Je suis une autre version modifiée du code de l'exo 1");
    >>>>>>> origin/main
        }
    }
    ```
    Éditez le fichier pour choisir la version finale (celle du dépôt distant) et supprimez les marqueurs (`<<<<<<<`, `=======`, `>>>>>>>`).

14. Marquons la résolution du conflit :

    ```sh
    ~/Dev-Objets/tp1$ git add Exercice1/HelloWorld.java
    ~/Dev-Objets/tp1$ git commit -m "Fusion des deux versions de HelloWorld"
    ```

15. Et publions sur le dépôt distant également pour que tout le monde soit synchronisé :

    ```sh
    ~/Dev-Objets/tp1$ git push origin main
    ```

16. Vérifier l'historique final
    ...
    ```sh
    *   xyz789 (HEAD -> main, origin/main) Fusion des deux versions
        |\  
        | * b11e5f6 Message du commit de copie_TP1
    * | 7423229 Message du commit sur le dépôt distant
      |/
    * 51a095c Commit précédent commun
    ```
    

Vous l'aurez compris : avoir des conflits entre les différents dépôts d'un même projet est un signe de mauvaise pratique de travail, qu'il faut absolument améliorer. Git est là pour vous le signaler... et vous montrer à quel point une mauvaise organisation d'un travail collaboratif peut être pénible à gérer.

**Remarque :** Notez que la commande `git pull` fait exactement `git fetch` + `git merge` :

```sh
# Version courte
git pull origin main

# Version longue (plus de contrôle)
git fetch origin
git merge origin/main
  ```
Avantage de `git fetch` + `git merge` :
   * Vous pouvez inspecter (`git log`, `git diff`) avant de fusionner
   * Plus de contrôle sur le processus
   * C'est plus clair pour comprendre ce qui se passe

#### Exercice 3 — Résolution de conflits à "l'arrache" (en écrasant tout)

Si le nombre de fichiers en conflit est grand (car vous vous êtes mal organisé avec vos collègues), et que vous voulez prendre la solution du dépôt distant en **écrasant** complétement ce que vous avez dans le dépôt local, alors vous pouvez utiliser l'arme de destruction (`git reset`) :
...
**Remarque** : dans votre cas, forcer un push sur votre fork GitLab vous sera refusé, parce que la branche (`main` ou `master`) est actuellement configurée pour être protégée (justement des "pushs forcés").
